...
class AACSound  extends flash.events.EventDispatcher {
	var input : URLStream;
	var nt : NetStream;
	var adtsStripper : AdtsStripper;
	var syncOk : Bool;
	var mustPlay:Bool;
	var url:String;
	...
	function convertStream( e : Event ) {		
		...
		if (input.bytesAvailable != 0){
			...
			adtsStripper.addData(data);
		}
		while(adtsStripper.bytesAvailable()>=8192){
			if ( !syncOk ){
				if (adtsStripper.findNextFrame()){
					syncOk = true;
					var header = FlvHeaderGenerator.getInstance().getHeader();
					nt.appendBytes(header);
				}
				continue;
			}
			var ff = adtsStripper.readBytes(1);	// Siempre debe ser 0xff
			if (ff[0] != 255) _on_error(new IOErrorEvent(IOErrorEvent.IO_ERROR,false,false,"Unrecognized AAC header arrived"));
			adtsStripper.parseADTSHeader();	
			var frameLength = adtsStripper.getFrameLength();
			var data = new ByteArray();
			data.writeBytes(adtsStripper.readBytes(frameLength - 1));
			var paquet = FlvHeaderGenerator.getInstance().genPaquet(data, adtsStripper.createDecoderSpecificInfo());
			nt.appendBytes(paquet);
		}
	}
