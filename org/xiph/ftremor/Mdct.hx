package org.xiph.ftremor;

class Mdct {

    /* At the moment we're using the low accuracy mode, because the
       lack of ready/builtin 64-bit int operations... */

    // #if lowacc
    inline static var cPI3_8 : Int = 0x0062;
    inline static var cPI2_8 : Int = 0x00b5;
    inline static var cPI1_8 : Int = 0x00ed;

    inline function MULT32(x : Int, y : Int) : Int {
        return (x >> 9) * y; // preshifted y >>23
    }
    inline function MULT31(x : Int, y : Int) : Int {
        return (x >> 8) * y; // preshifted y >>23
    }
    inline function MULT31_SHIFT15(x : Int, y : Int) : Int {
        return (x >> 6) * y; // preshifted y >>9
    }
    // #end


    inline function XPROD32(a : Int, b : Int, t : Int, v : Int,
			   x : Array<Int>, ix : Int,
                           y : Array<Int>, iy : Int)
    {
        x[ix] = MULT32(a, t) + MULT32(b, v);
        y[iy] = MULT32(b, t) - MULT32(a, v);
    }

    inline function XPROD31(a : Int, b : Int, t : Int, v : Int,
                            x : Array<Int>, ix : Int,
                            y : Array<Int>, iy : Int)
    {
        x[ix] = MULT31(a, t) + MULT31(b, v);
        y[iy] = MULT31(b, t) - MULT31(a, v);
    }

    inline function XNPROD31(a : Int, b : Int, t : Int, v : Int,
                             x : Array<Int>, ix : Int,
                             y : Array<Int>, iy : Int)
    {
        x[ix] = MULT31(a, t) - MULT31(b, v);
    ...
    inline
    function butterfly_8(x : Array<Int>, i : Int) : Void {
        var r0 : Int = x[i + 4] + x[i + 0];
        var r1 : Int = x[i + 4] - x[i + 0];
        var r2 : Int = x[i + 5] + x[i + 1];
        var r3 : Int = x[i + 5] - x[i + 1];
        var r4 : Int = x[i + 6] + x[i + 2];
        var r5 : Int = x[i + 6] - x[i + 2];
        var r6 : Int = x[i + 7] + x[i + 3];
        var r7 : Int = x[i + 7] - x[i + 3];
    ...
    inline
    function butterfly_16(x : Array<Int>, i : Int) : Void {
        var r0 : Int;
        var r1 : Int;

        r0 = x[i + 0] - x[i + 8]; x[i + 8] += x[i + 0];
        r1 = x[i + 1] - x[i + 9]; x[i + 9] += x[i + 1];
        x[i + 0] = MULT31((r0 + r1), cPI2_8);
        x[i + 1] = MULT31((r1 - r0), cPI2_8);

        r0 = x[i + 10] - x[i + 2]; x[i + 10] += x[i + 2];
        r1 = x[i + 3] - x[i + 11]; x[i + 11] += x[i + 3];
        x[i + 2] = r1; x[i + 3] = r0;

        r0 = x[i + 12] - x[i + 4]; x[i + 12] += x[i + 4];
        r1 = x[i + 13] - x[i + 5]; x[i + 13] += x[i + 5];
        x[i + 4] = MULT31((r0 - r1), cPI2_8);
        x[i + 5] = MULT31((r0 + r1), cPI2_8);

        r0 = x[i + 14] - x[i + 6]; x[i + 14] += x[i + 6];
        r1 = x[i + 15] - x[i + 7]; x[i + 15] += x[i + 7];
        x[i + 6] = r0; x[i + 7] = r1;
    ...
    inline
    function butterfly_32(x : Array<Int>, i : Int) : Void {
        var r0 : Int;
        var r1 : Int;

        r0 = x[i + 30] - x[i + 14]; x[i + 30] += x[i + 14];
        r1 = x[i + 31] - x[i + 15]; x[i + 31] += x[i + 15];
        x[i + 14] = r0; x[i + 15] = r1;

        r0 = x[i + 28] - x[i + 12]; x[i + 28] += x[i + 12];
        r1 = x[i + 29] - x[i + 13]; x[i + 29] += x[i + 13];
        XNPROD31(r0, r1, cPI1_8, cPI3_8, x, i + 12, x, i + 13);

        r0 = x[i + 26] - x[i + 10]; x[i + 26] += x[i + 10];
        r1 = x[i + 27] - x[i + 11]; x[i + 27] += x[i + 11];
        x[i + 10] = MULT31((r0 - r1) , cPI2_8);
        x[i + 11] = MULT31((r0 + r1) , cPI2_8);

        r0 = x[i + 24] - x[i + 8]; x[i + 24] += x[i + 8];
        r1 = x[i + 25] - x[i + 9]; x[i + 25] += x[i + 9];
        XNPROD31(r0, r1, cPI3_8, cPI1_8, x, i + 8, x, i + 9);

        r0 = x[i + 22] - x[i + 6]; x[i + 22] += x[i + 6];
        r1 = x[i + 7] - x[i + 23]; x[i + 23] += x[i + 7];
        x[i + 6] = r1; x[i + 7] = r0;

        r0 = x[i + 4] - x[i + 20]; x[i + 20] += x[i + 4];
        r1 = x[i + 5] - x[i + 21]; x[i + 21] += x[i + 5];
        XPROD31(r0, r1, cPI3_8, cPI1_8, x, i + 4, x, i + 5);

        r0 = x[i + 2] - x[i + 18]; x[i + 18] += x[i + 2];
        r1 = x[i + 3] - x[i + 19]; x[i + 19] += x[i + 3];
        x[i + 2] = MULT31((r1 + r0) , cPI2_8);
        x[i + 3] = MULT31((r1 - r0) , cPI2_8);

        r0 = x[i + 0] - x[i + 16]; x[i + 16] += x[i + 0];
        r1 = x[i + 1] - x[i + 17]; x[i + 17] += x[i + 1];
        XPROD31(r0, r1, cPI1_8, cPI3_8, x, i /*+ 0*/, x, i + 1);
    ...
    /* inline */
    function butterfly_generic(x : Array<Int>, i : Int, points : Int,
                               step : Int) : Void
    {
        var T : Array<Int> = sincos_lookup0;
        var iT0 : Int = 0;
        var iT1 : Int = 1;
        var i1 : Int = i + (points - 8);
        var i2 : Int = i + ((points >> 1) - 8);

        var r0 : Int;
        var r1 : Int;

        do {
            r0 = x[i1 + 6] - x[i2 + 6]; x[i1 + 6] += x[i2 + 6];
            r1 = x[i2 + 7] - x[i1 + 7]; x[i1 + 7] += x[i2 + 7];
            XPROD31(r1, r0, T[iT0], T[iT1], x, i2 + 6, x, i2 + 7);
            iT0 += step; iT1 += step;

            r0 = x[i1 + 4] - x[i2 + 4]; x[i1 + 4] += x[i2 + 4];
            r1 = x[i2 + 5] - x[i1 + 5]; x[i1 + 5] += x[i2 + 5];
            XPROD31(r1, r0, T[iT0], T[iT1], x, i2 + 4, x, i2 + 5);
            iT0 += step; iT1 += step;

            r0 = x[i1 + 2] - x[i2 + 2]; x[i1 + 2] += x[i2 + 2];
            r1 = x[i2 + 3] - x[i1 + 3]; x[i1 + 3] += x[i2 + 3];
            XPROD31(r1, r0, T[iT0], T[iT1], x, i2 + 2, x, i2 + 3);
            iT0 += step; iT1 += step;

            r0 = x[i1 + 0] - x[i2 + 0]; x[i1 + 0] += x[i2 + 0];
            r1 = x[i2 + 1] - x[i1 + 1]; x[i1 + 1] += x[i2 + 1];
            XPROD31(r1, r0, T[iT0], T[iT1], x, i2 + 0, x, i2 + 1);
            iT0 += step; iT1 += step;
        ...
        do {
            r0 = x[i1 + 6] - x[i2 + 6]; x[i1 + 6] += x[i2 + 6];
            r1 = x[i1 + 7] - x[i2 + 7]; x[i1 + 7] += x[i2 + 7];
            XNPROD31(r0, r1, T[iT0], T[iT1], x, i2 + 6, x, i2 + 7);
            iT0 -= step; iT1 -= step;

            r0 = x[i1 + 4] - x[i2 + 4]; x[i1 + 4] += x[i2 + 4];
            r1 = x[i1 + 5] - x[i2 + 5]; x[i1 + 5] += x[i2 + 5];
            XNPROD31(r0, r1, T[iT0], T[iT1], x, i2 + 4, x, i2 + 5);
            iT0 -= step; iT1 -= step;

            r0 = x[i1 + 2] - x[i2 + 2]; x[i1 + 2] += x[i2 + 2];
            r1 = x[i1 + 3] - x[i2 + 3]; x[i1 + 3] += x[i2 + 3];
            XNPROD31(r0, r1, T[iT0], T[iT1], x, i2 + 2, x, i2 + 3);
            iT0 -= step; iT1 -= step;

            r0 = x[i1 + 0] - x[i2 + 0]; x[i1 + 0] += x[i2 + 0];
            r1 = x[i1 + 1] - x[i2 + 1]; x[i1 + 1] += x[i2 + 1];
            XNPROD31(r0, r1, T[iT0], T[iT1], x, i2 + 0, x, i2 + 1);
            iT0 -= step; iT1 -= step;
        ...
        do {
            r0 = x[i2 + 6] - x[i1 + 6]; x[i1 + 6] += x[i2 + 6];
            r1 = x[i2 + 7] - x[i1 + 7]; x[i1 + 7] += x[i2 + 7];
            XPROD31(r0, r1, T[iT0], T[iT1], x, i2 + 6, x, i2 + 7);
            iT0 += step; iT1 += step;

            r0 = x[i2 + 4] - x[i1 + 4]; x[i1 + 4] += x[i2 + 4];
            r1 = x[i2 + 5] - x[i1 + 5]; x[i1 + 5] += x[i2 + 5];
            XPROD31(r0, r1, T[iT0], T[iT1], x, i2 + 4, x, i2 + 5);
            iT0 += step; iT1 += step;

            r0 = x[i2 + 2] - x[i1 + 2]; x[i1 + 2] += x[i2 + 2];
            r1 = x[i2 + 3] - x[i1 + 3]; x[i1 + 3] += x[i2 + 3];
            XPROD31(r0, r1, T[iT0], T[iT1], x, i2 + 2, x, i2 + 3);
            iT0 += step; iT1 += step;

            r0 = x[i2 + 0] - x[i1 + 0]; x[i1 + 0] += x[i2 + 0];
            r1 = x[i2 + 1] - x[i1 + 1]; x[i1 + 1] += x[i2 + 1];
            XPROD31(r0, r1, T[iT0], T[iT1], x, i2 + 0, x, i2 + 1);
            iT0 += step; iT1 += step;
        ...
        do{
            r0 = x[i1 + 6] - x[i2 + 6]; x[i1 + 6] += x[i2 + 6];
            r1 = x[i2 + 7] - x[i1 + 7]; x[i1 + 7] += x[i2 + 7];
            XNPROD31(r1, r0, T[iT0], T[iT1], x, i2 + 6, x, i2 + 7);
            iT0 -= step; iT1 -= step;

            r0 = x[i1 + 4] - x[i2 + 4]; x[i1 + 4] += x[i2 + 4];
            r1 = x[i2 + 5] - x[i1 + 5]; x[i1 + 5] += x[i2 + 5];
            XNPROD31(r1, r0, T[iT0], T[iT1], x, i2 + 4, x, i2 + 5);
            iT0 -= step; iT1 -= step;

            r0 = x[i1 + 2] - x[i2 + 2]; x[i1 + 2] += x[i2 + 2];
            r1 = x[i2 + 3] - x[i1 + 3]; x[i1 + 3] += x[i2 + 3];
            XNPROD31(r1, r0, T[iT0], T[iT1], x, i2 + 2, x, i2 + 3);
            iT0 -= step; iT1 -= step;

            r0 = x[i1 + 0] - x[i2 + 0]; x[i1 + 0] += x[i2 + 0];
            r1 = x[i2 + 1] - x[i1 + 1]; x[i1 + 1] += x[i2 + 1];
            XNPROD31(r1, r0, T[iT0], T[iT1], x, i2 + 0, x, i2 + 1);
            iT0 -= step; iT1 -= step;
    ...
    inline function butterflies(x : Array<Int>, i : Int, points : Int,
                                shift : Int) : Void
    {
        var stages : Int = 8 - shift;
        var j : Int, k : Int;
        ...
        while (--stages > 0) {
            k = 0;
            while (k < (1<<j)) {
                butterfly_generic(x, i + (points >> j) * k, points >> j,
                                  4 << (j + shift));
                k++;
            }
            j++;
    ...
    static inline var bitrev : Array<Int> = [0, 8, 4, 12, 2, 10, 6, 14,
                                             1, 9, 5, 13, 3, 11, 7, 15];

    inline function bitrev12(x : Int) : Int {
        return bitrev[x>>8] | (bitrev[(x & 0x0f0) >> 4] << 4) |
            (bitrev[x & 0x00f] << 8);
    }

    /* inline */
    function bitreverse(x : Array<Int>, i : Int, n : Int, step : Int,
                        shift : Int) : Void
    {
        ...
        do {
            var r3 : Array<Int> = [bitrev12(bit++)];
            var i0 : Int = i + ((r3[0] ^ 0xfff) >> shift) - 1;
            var i1 : Int = i + (r3[0] >> shift);

            var r0 : Int = x[i0 /* + 0*/] + x[i1 /* + 0*/];
            var r1 : Int = x[i1 + 1] - x[i0 + 1];

            XPROD32(r0, r1, T[iT1], T[iT0], r2, 0, r3, 0);
            iT0 += step; iT1 += step;
        ...
        do {
            ...
            var r0 : Int = x[i0] + x[i1];
            var r1 : Int = x[i1 + 1] - x[i0 + 1];

            iT0 -= step; iT1 -= step;
            XPROD32(r0, r1, T[iT0], T[iT1], r2, 0, r3, 0);
    ...
    public function
    backward(n : Int, in_ : Array<Int>, out : Array<Int>) : Void
    {
        var i : Int = 0;
        var o : Int = 0;
        ...
        do {
            oX -= 4;
            XPROD31(in_[iX + 4], in_[iX + 6], T[iT0], T[iT1],
                    out, oX + 2, out, oX + 3);
            iT0 += step; iT1 += step;
            XPROD31(in_[iX + 0], in_[iX + 2], T[iT0], T[iT1],
                    out, oX + 0, out, oX + 1);
            iT0 += step; iT1 += step;
            iX -= 8;
        } while (iX >= i + n4);
        ...
        do {
            oX -= 4;
            XPROD31(in_[iX + 4], in_[iX + 6], T[iT1], T[iT0],
                    out, oX + 2, out, oX + 3);
            iT0 -= step; iT1 -= step;
            XPROD31(in_[iX + 0], in_[iX + 2], T[iT1], T[iT0],
                    out, oX + 0, out, oX + 1);
            iT0 -= step; iT1 -= step;
            iX -= 8;
        } while (iX >= i);
        ...
        do {
            iT0 += step; iT1 += step;
            XPROD31(in_[iX + 6], in_[iX + 4], T[iT0], T[iT1],
                    out, oX + 0, out, oX + 1);
            iT0 += step; iT1 += step;
            XPROD31(in_[iX + 2], in_[iX + 0], T[iT0], T[iT1],
                    out, oX + 2, out, oX + 3);
            iX -= 8;
            oX += 4;
        } while (iX >= i + n4);
        ...
        do {
            iT0 -= step; iT1 -= step;
            XPROD31(in_[iX + 6], in_[iX + 4], T[iT1], T[iT0],
                    out, oX + 0, out, oX + 1);
            iT0 -= step; iT1 -= step;
            XPROD31(in_[iX + 2], in_[iX + 0], T[iT1], T[iT0],
                    out, oX + 2, out, oX + 3);
            iX -= 8;
            oX += 4;
        } while (iX >= i);

        butterflies(out, o + n2, n2, shift);
        bitreverse(out, o, n, step, shift);
        ...
        step >>= 2;
        {
            ...
            default: {
                ...
                do {
                    oX1 -= 4;
                    XPROD31(out[iX + 0], - out[iX + 1], T[iT0], T[iT1],
                            out, oX1 + 3, out, oX2 + 0);
                    iT0 += step; iT1 += step;
                    XPROD31(out[iX + 2], - out[iX + 3], T[iT0], T[iT1],
                            out, oX1 + 2, out, oX2 + 1);
                    iT0 += step; iT1 += step;
                    XPROD31(out[iX + 4], - out[iX + 5], T[iT0], T[iT1],
                            out, oX1 + 1, out, oX2 + 2);
                    iT0 += step; iT1 += step;
                    XPROD31(out[iX + 6], - out[iX + 7], T[iT0], T[iT1],
                            out, oX1 + 0, out, oX2 + 3);
                    iT0 += step; iT1 += step;
            ...
            case 1: {
                /* linear interpolation between table values:
                   offset=0.5, step=1 */
                var t0 : Int, t1 : Int, v0 : Int, v1 : Int;
                T = sincos_lookup0;
                V = sincos_lookup1;
                var iT : Int = 0;
                var iV : Int = 0;
                ...
                do {
                    ...
                    t0 += (v0 = (V[iV++]) >> 1);
                    t1 += (v1 = (V[iV++]) >> 1);
                    XPROD31(out[iX + 0], - out[iX + 1], t0, t1,
                            out, oX1 + 3, out, oX2 + 0);
                    v0 += (t0 = (T[iT++]) >> 1);
                    v1 += (t1 = (T[iT++]) >> 1);
                    XPROD31(out[iX + 2], - out[iX + 3], v0, v1,
                            out, oX1 + 2, out, oX2 + 1);
                    t0 += (v0 = (V[iV++]) >> 1);
                    t1 += (v1 = (V[iV++]) >> 1);
                    XPROD31(out[iX + 4], - out[iX + 5], t0, t1,
                            out, oX1 + 1, out, oX2 + 2);
                    v0 += (t0 = (T[iT++]) >> 1);
                    v1 += (t1 = (T[iT++]) >> 1);
                    XPROD31(out[iX + 6], - out[iX + 7], v0, v1,
                            out, oX1 + 0, out, oX2 + 3);
            ...
            case 0: {
                /* linear interpolation between table values:
                   offset=0.25, step=0.5 */
                var t0 : Int, t1 : Int, v0 : Int, v1 : Int, q0 : Int, q1 : Int;
                T = sincos_lookup0;
                V = sincos_lookup1;
                var iT : Int = 0;
                var iV : Int = 0;
                ...
                do {
                    ...
                    t0  = v0 - q0;
                    t1  = v1 - q1;
                    XPROD31(out[iX + 2], - out[iX + 3], t0, t1,
                            out, oX1 + 2, out, oX2 + 1);
                    ...
                    v0 += (q0 = (t0 - v0) >> 2);
                    v1 += (q1 = (t1 - v1) >> 2);
                    XPROD31(out[iX + 4], - out[iX + 5], v0, v1,
                            out, oX1 + 1, out, oX2 + 2);
                    v0  = t0 - q0;
                    v1  = t1 - q1;
                    XPROD31(out[iX + 6], - out[iX + 7], v0, v1,
    ...
    // #if lowacc
    static inline function X(n : Int) : Int {
        return (((n >> 22) + 1) >> 1) - (((n >> 22) + 1) >> 9);
    }
    // #end

    public function init_lookup() : Void {
        sincos_lookup0 = _sincos_lookup0;
        sincos_lookup1 = _sincos_lookup1;
    }

    var sincos_lookup0 : Array<Int>;
    var sincos_lookup1 : Array<Int>;

    /* {sin(2*i*PI/4096), cos(2*i*PI/4096)}, with i = 0 to 512 */
    static var _sincos_lookup0 : Array<Int> = [
    ...
    static var _sincos_lookup1 : Array<Int> = [
